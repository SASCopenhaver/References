...
<cfoutput>
	<tr class="c_fieldValue">
		<td scope="col"><!--- <input type="checkbox" name="cbx_job_almn_id_#job_almn_id#_#job_num#" value="#job_almn_id#"> --->
				<input type="checkbox" name="cbx_job_almn_id_#local.LoopNum#" value="#local.LoopNum#">
		</td>
		<td>#job_almn_id#</td>
		<td><input type="text" class="forminputs" name="txt_job_num_#local.LoopNum#" value="#job_num#"></td>
...
function f_assignJobNum(){with(self.document.forms[0]){
	...
	{   //cbx_job_almn_id_
		...
		{ 
			alert(e)
			//vLoopNum = elements[e].name.substr(16); 
			//eval( "txt_job_num_"+vLoopNum+".value="+vLoopNum );
		}
...
var vLoopNum = 0;//f_assignJobNum();
	...
	{   //cbx_job_almn_id_
		...
		{ 
			...
			vLoopNum = elements[e].name.substr(16)); 
			//
			alert(elements[e].name.substr(0,16))
			
			eval( "txt_job_num_"+vLoopNum+".value="+vLoopNum );
			//
			if ( eval("txt_job_title_txt_"+vLoopNum+".value == ''") )
			{
				vValidationPassed = "NO";
				alert("Sorry, 'Job Title' must be provided for this record.");
				eval("txt_job_title_txt_"+vLoopNum+".focus();");
				break;
			}
