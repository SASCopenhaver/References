	...
	if (vCounter != 0)	
	{
			...
			/*
					vOutputPV += "<td colspan=\"7\">";
						vOutputPV += "<table align=\"left\" width=\"60%\" border=\"0\" style=\"border-bottom:1px solid Gray;\">";
						vOutputPV += "<tr><th width=\"40%\" align=\"right\">PLACES VISITED</th><th width=\"20%\" align=\"right\"> </th></tr>";
						vOutputPV += "<tr><td colspan=\"2\" class=\"c_fieldValueRed\" align=\"left\">Only selected \"Places Visited\" will remain in database. All not selected \"Places Visited\" will be permanently deleted. </td></tr>";
						vOutputPV += "</table><br>";
					vOutputPV += "</td>";
			*/
					vOutputPV += "<td colspan=\"7\">";
			...
			vOutputPV += "<tr>";
					...
					vOutputPV += "<th>Country</th>";//aplace_visit_cntry_nm
		...
		if (elements[e].name.substr(0,13) == "cbx_pgm_almn_" && elements[e].checked == true)
		{
			for (var p=0; p<orgBfrLOP.length; p++)
			{
					...
					)
				{
					...
					//
					//if (argColumnNm == "pgm_type_nm")      { h_pgm_type_cd.value       = orgBfrLOP[p].apgm_type_cd }
					//if (argColumnNm == "pgm_from_cntry_nm"){ h_pgm_from_cntry_cd.value = orgBfrLOP[p].apgm_from_cntry_cd }
					//if (argColumnNm == "pgm_to_cntry_nm")  { h_pgm_to_cntry_cd.value   = orgBfrLOP[p].apgm_to_cntry_cd }
					//
			...
			//=================================================================================================================================
				   ...
				   vOutput += "<table width=\"100%\" border=\"0\">";
					...
					vOutput += "<tr>";
							...
							vOutput += "<td align=\"left\" width=\"10%\">";
							vOutput += "<input type=\"text\" id=\"\" name=\"txt_mrgd_pgm_field_stdy_nm\" class=\"forminputs\" size=\"33\" value=\"\">";
							vOutput += "</td>";
					vOutput += "</tr>";
					vOutput += "<tr>";
...
function f_setMergedData(){with(self.document.forms[0]){
		...
		f_loopCheckingValues("pgm_type_nm", "ddlb_"); 
		f_loopCheckingValues("pgm_fy_dt", "txt_"); 
	
	//f_loopCheckingValues("pgm_yr_dt"); 
	 
		//f_loopCheckingValues("pgm_from_cntry_nm", "ddlb_"); 
		//f_loopCheckingValues("pgm_to_cntry_nm", "ddlb_"); 
	/*f_loopCheckingValues("pgm_start_dt"); 
	f_loopCheckingValues("pgm_end_dt"); 
	f_loopCheckingValues("pgm_field_stdy_nm");  
	f_loopCheckingValues("pgm_agcy_nm"); 
	f_loopCheckingValues("pgm_plcmnt_org_nm");  
	f_loopCheckingValues("pgm_nmtng_org_nm"); 
	f_loopCheckingValues("pgm_sub_ctgry_txt"); 
	f_loopCheckingValues("pgm_topic_txt"); 
	f_loopCheckingValues("pgm_strtgc_goal_txt");  
	f_loopCheckingValues("pgm_spcl_award_txt"); 
	f_loopCheckingValues("pgm_updt_dt"); 
	f_loopCheckingValues("pgm_updt_id"); 
	f_loopCheckingValues("pgm_src_nm"); 
	*/
...
//======================================================================================================================================
	...
	else if (vCounter > 1)//====================================================================================================================
	{
			...
			//else if (argClmnNm == "pgm_type_nm" &&  )
			//{//alert("3   "+vVal_1+"   "+vVal_2)
			//		eval(vCtrlNm+".checked = false");
			//		of_selectedInDDLB ("ddlb_mrgd_"+argClmnNm, "-1");
			//}
			//
...
//======================================================================================================================================
	...
	//, "", "apgm_type_crnt_cd", "apgm_type_nm",
			
			/*
			for (var t=0; t<rowsPT; t++)
			{
				if ( eval("orgBfrPT[t].a"+argClmnNm+" == elements[e].value") )
				{   //alert(eval("orgBfrPT[t].a"+argClmnNm))
					if (vVal_1 == "" ){ vVal_1 = elements[e].value; vCounter++; vCheck = e; vT = t;}
					if (vCounter >= 1){ vVal_2 = elements[e].value; vCounter++; }
					if (vVal_1 != vVal_2 ){ vCounter = 0; break; }
				}
			}
			*/
	...
	//
	/*
	if (vCounter == 1 || (vCounter > 1 && vVal_1 == vVal_2))
	{
		elements[vCheck].checked = true;
		//
		switch (argMergedCtrlType)
		{
			case"ddlb_":
			
			
				if ("ddlb_mrgd_"+argClmnNm == "ddlb_mrgd_pgm_type_nm")
				{ for (var pt=0; pt<rowsPT; pt++) {if(orgBfrPT[pt].apgm_type_nm == vVal_1) { vTemp = orgBfrPT[pt].apgm_type_crnt_cd;}}}
				//
				//"ddlb_mrgd_pgm_from_cntry_nm", "-1", "=== Select Country ===", rowsConCurr, "orgBfrConCurr", "acntry_cntry_cd", "acntry_cntry_nm"
				
				
				
				if ("ddlb_mrgd_"+argClmnNm == "ddlb_mrgd_pgm_from_cntry_nm")
				{//alert("ddlb_mrgd_"+argClmnNm+"   "+vVal_1) 
				for (var c=0; c<rowsConCurr; c++){ if(orgBfrConCurr[c].acntry_cntry_nm == vVal_1){vTemp = orgBfrConCurr[c].acntry_cntry_cd;}}}
			...
			//
			case"txt_":
				vVal_1 = f_LaundryText(vVal_1);
				//
				eval("txt_mrgd_"+argClmnNm+".value='"+vVal_1+"'");
			break;
		}
	}
	*/
	//
	//vCounter = 0; vVal_1 = ""; vVal_2 = ""; vCheck = 0;
	//
}}
//
//------------------------------------------------------------------------------------------------
...
{//alert(argVal+"   "+argCtrlNm);
	...
	//
	vStrTemp = vPartOfCtrlNm.substr(4, vPartOfCtrlNm.length);//"pgm_id_"
	//alert(vStrTemp)
	...
	if (vTrueFalse == true)
	{
		...
		//alert("txt_mrgd_"+vStrTemp+".value=  "+argVal)
