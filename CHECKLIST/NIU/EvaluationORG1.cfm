...
//******************************************************************************
	function onChEvalStat(argEvalStat){with(self.document.forms[0]){
		...
		//alert("onChEvalStat   "+argEvalStat)
		
		onChPrjNum(-1);
		//
		//-- The following applies to s_PrjNum
		var p = 0;
		arrayTemp.length = 0;
		if (argEvalStat == -1)
		{
...
//******************************************************************************
	...
	var vSgoal, cSnpaOfficer;
		...
		//
		//setAnswers(argPrjNum);
	}}//end of with and function


//******************************************************************************

	function setAnswers(argPrjNum){with(self.document.forms[0]){
	
		alert("setAnswers  "+argPrjNum);
		
		vTemp = "";
		for (var i=0; i<rowsUP; i++)
		{
			if (argPrjNum == arrayUP[i].aPrjNum)
			{
				vTemp = arrayUP[i].aPrjID;
				//alert(vTemp)
				break;
			}
		}
		...
		//----------------------------------------------------------------------
		//aQ_101.options[2].selected = true;
		//
		//----------------------------------------------------------------------
	//
